...
import { useAddPurchaseVoucherMutation } from "../../Redux/Api/vouchers/Purchase";
...
const [addPurchaseVoucher]= useAddPurchaseVoucherMutation();
  // Fetch company names
  ...
  const handlePurchase = async (values) => {
    console.log(values);
    try {
      const response = await addPurchaseVoucher(values);
  ...
  const initialValues = {
    ...
    additionalCharges: [{ reason: "", charges: 0.0 }],
    // totalAmount: "",
    // GrandTotalAmount: "",
    currency: "INR",
  };
            ...
            <Form className="row g-3">
              ...
              <div className="col-md-6">
                ...
                >
                  ...
                  {companyNames?.items?.map((company, i) => (
              ...
              <div className="col-md-6">
                ...
                <Field as="select" name="seller" className="form-control">
                  ...
                  {companyNames?.items
                    ?.filter((company) => company._id !== values.purchasedBy)
                    .map((company, i) => (
                      <option
                        key={i}
                        value={company._id}
                        disabled={company._id === values.purchasedBy}
                      >
                        {company.name || "Unnamed Company"}
                      </option>
                    ))}
                  ...
                  {({ field }) => (
                    <Select
                      ...
                      value={values.itemId.map((s) => ({
                        value: s,
                        label: itemId.find((sec) => sec._id === s).itemDetails
                          .name,
                      }))}
                      onChange={(itemId) =>
                        setFieldValue(
                          "itemId",
                          itemId.map((s) => s.value)
                        )
                      }
              ...
              <div>
                <Field
                  ...
                  onChange={(e) =>
                    setFieldValue("isShippingPaid", e.target.checked)
                  }
                />
