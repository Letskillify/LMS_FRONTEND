...
} from "../../Redux/Api/Stock/AccountSlice";
...
const validationSchema = Yup.object().shape({
  ...
  gst: Yup.object().shape({
    ...
    gstNumber: Yup.string().when("type", (type, schema) => {
      if (type === "Regular" || type === "Composition") {
        return schema
          .matches(
  ...
  const handleApi = async (formData) => {
    try {
      ...
      if (response.data.status === 201) {
        showToast("Firm account created successfully", 'success');
        fatchData();
      }
    } catch (err) {
      console.error(err.response ? err.response.data : err.message);
      showToast(err.response?.data?.message || "Error adding Firm Account",'error');
    }
  };
  const { data: FirmAccount } = useGetFirmAccountsByInstituteIdQuery(
    InstituteId,
    { skip: !InstituteId }
  );
  const [createFirmAccount] = useCreateFirmAccountMutation();
  const [deleteFirmAccount] = useDeleteFirmAccountMutation();
  const [updateFirmAccount] = useUpdateFirmAccountMutation();
  useEffect(() => {
    if (InstituteId) {
      setForms(FirmAccount?.items);
    }
  }, [InstituteId, FirmAccount]);

  console.log("FirmAccount", FirmAccount);

  const handleDelete = async (id) => {
      try {
          const res = await deleteFirmAccount(id);
          if (res.status === 200) {
              showToast("Firm account deleted successfully", 'success');
              fatchData();
          }

      } catch (err) {
          showToast(err.response?.data?.message || "Error deleting Firm Account", 'error');
      }
  };
  const handleEdit = async (v, id) => {
      try {
          const res = await updateFirmAccount({ id, data: v });
          if (res.status === 201) {
              showToast("Inventory edding successfully",'success');
          }
      } catch (err) {
          showToast(err.response.data.message || "Error edding inventory", 'error')
      };
  }
  return (
    <>
                                ...
                                <div className="row mb-3">
                                  <div className="col-md-6">
                                    ...
                                    <Field
                                      as="select"
                                      className="form-control"
                                      name="businessDetails.industryType"
                                    >
                                      <option value="">
                                        Select Industry Type
                                      </option>
                                      ...
                                      <option value="Fishing & Aquaculture">
                                        Fishing & Aquaculture
                                      </option>
                                      ...
                                      <option value="Energy (Oil, Gas, Renewable)">
                                      ...
                                      <option value="Food Processing & Beverages">
                                        Food Processing & Beverages
                                      </option>
                                      <option value="Machinery & Industrial Equipment">
                                        Machinery & Industrial Equipment
                                      </option>
                                      <option value="Textiles & Apparel">
                                        Textiles & Apparel
                                      </option>
                                      <option value="Steel & Metalworks">
                                        Steel & Metalworks
                                      </option>
                                      <option value="Plastic & Polymer Manufacturing">
